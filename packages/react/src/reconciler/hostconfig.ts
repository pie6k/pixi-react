...
function hideInstance<Container extends PixiReactMinimalExpandoContainer>(
    instance: Container
): void
{
...
function unhideInstance<Container extends PixiReactMinimalExpandoContainer>(
    instance: Container,
    props: PropsType
): void
{
...
function attach<
    Container extends PixiReactMinimalExpandoContainer,
    Instance extends PixiReactMinimalExpandoContainer
>(parent: Container, child: Instance, type: AttachType<Container, Instance>)
{
...
>(child: Instance, parent: Container)
{
    ...
    if (child.config?.destroyChildren !== false && child.children?.length)
    {
        ...
        const unmountingChildren = [
            ...child.children,
            ...(child.__reactpixi?.attachedObjects || []),
        ];
...
>(parent: Container, child: Instance)
{
    ...
    {
        parent.__reactpixi.attachedObjects
            = parent.__reactpixi.attachedObjects.filter((x) => x !== child);
    }
    if (child.__reactpixi?.attach)
    {
    ...
    if (child.__reactpixi)
    {
        ...
        delete (child.__reactpixi as Partial<LocalState<any, any>>)
            .attachedObjects;
    }
...
>(
    container: Container,
    child: Instance,
    beforeChild: Instance | SiblingInstance
)
{
...
}): GenericPixiReactHostConfig<Container, Instance>
{
    let prepareChanged: UpdatePayload = null;

    function prepareUpdate(
        pixiElement: Instance,
        type: string,
        oldProps: PropsType,
        newProps: PropsType
    )
    ...
    function createElement(
        type: string,
        props: InstanceProps<Container, Instance>,
        root: Container
    ): Instance
    {
        ...
        {
            const customComponentLifecycle = componentLifecycle as ICustomComponent<
            PropsType,
            Container,
            Instance
            >;
            ...
            {
                instance = (
                    componentLifecycle as createCustomComponentType<
                    PropsType,
                    Container,
                    Instance
                    >
